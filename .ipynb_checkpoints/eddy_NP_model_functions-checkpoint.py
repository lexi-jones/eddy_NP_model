...
def animate_eddy_NP_sim(Lx,Ly,del_x,del_y,del_t,skip,numP,size,psi_2D,all_P,N_2D):
    """
    Lx,Ly: number of grid cells
    del_x,del_y: size of grid cell in meters
    del_t: time step
    skip: number of timesteps to skip per frame in the animation
    numP: number of phytoplankton in simulation
    psi_2D: streamfunction
    all_P: phytoplankton concentration
    N_2D: nutrient concentration
    """
    
    xaxis_stream = np.arange(0,(Lx*del_x)/1000,del_x/1000) # x and y labels in km
    yaxis_stream = np.arange(0,(Ly*del_y)/1000,del_y/1000)
    
    # Tracer is on a smaller grid than the streamfunction
    xaxis_tracer = np.arange(0+(2*del_x)/1000,((Lx-1)*del_x)/1000,del_x/1000) # x and y labels in km
    yaxis_tracer = np.arange(0+(2*del_y)/1000,((Ly-1)*del_y)/1000,del_y/1000)

    fig, ax = plt.subplots(1,numP+2,figsize=(7*(numP+2),5))

    stream = ax[0].pcolormesh(xaxis_stream,yaxis_stream,psi_2D[0],vmin=-20000,vmax=0)
    ax[0].set_title('Streamfunction ($\psi$)',fontsize=18)
    ax[0].set_xlabel('Distance (km)',fontsize=18)
    ax[0].set_ylabel('Distance (km)',fontsize=18)
    cbar0 = fig.colorbar(stream, ax=ax[0])
    cbar0.set_label('$\psi$',rotation=270,fontsize=18)
    cbar0.ax.get_yaxis().labelpad = 20

    for n in np.arange(0,numP):
        if n == 0:
            phyto1 = ax[1+n].pcolormesh(xaxis_tracer,yaxis_tracer,all_P[0][0,1:-1,1:-1]/1000,vmin=0,vmax=(np.nanmax(all_P[n])/1000))
            cbar = fig.colorbar(phyto1, ax=ax[1+n])
        else:
            phyto2 = ax[1+n].pcolormesh(xaxis_tracer,yaxis_tracer,all_P[0][0,1:-1,1:-1]/1000,vmin=0,vmax=(np.nanmax(all_P[n])/1000))
            cbar = fig.colorbar(phyto2, ax=ax[1+n])

        ax[1+n].set_title('%s Phytoplankton\nConcentration'%(size[n]),fontsize=18)
        ax[1+n].set_xlabel('Distance (km)',fontsize=18)
        #ax[1+n].set_ylabel('Distance (km)',fontsize=18)
        cbar.set_label('$P (\mu M N)$', rotation=270,fontsize=18)
        cbar.ax.get_yaxis().labelpad = 20

    nut = ax[-1].pcolormesh(xaxis_tracer,yaxis_tracer,N_2D[0,1:-1,1:-1]/1000,vmin=0.0,vmax=(np.nanmax(N_2D)/1000)*3)
    ax[-1].set_title('Nutrient Concentration',fontsize=18)
    ax[-1].set_xlabel('Distance (km)',fontsize=18)
    #ax[-1].set_ylabel('Distance (km)',fontsize=18)
    cbar2 = fig.colorbar(nut, ax=ax[-1])
    cbar2.set_label('$N (\mu M N)$', rotation=270,fontsize=18)
    ...
    def animate_NP(i):
        ...
        ax[1].text(0.5, 1.1, "Timestep: %s (%s s)"%(i*skip,del_t),fontsize=18,
    ...
    def animate_NPP(i):
        ...
        nut.set_array((N_2D[i*skip,1:-1,1:-1]/1000).ravel())
        ax[2].text(2.75, 1.1, "Timestep: %s (%s s)"%(i*skip,del_t),fontsize=18,
                bbox={'facecolor': 'white'}, #,'alpha': 0.5, 'pad': 5
                transform=ax[0].transAxes, ha="center")
        return stream,phyto1,phyto2,nut

        plt.suptitle("Timestep: %s (%s s)"%(i*skip,del_t))

    # Run the animation
    if numP == 1:
        anim = FuncAnimation(fig, animate_NP, interval=200, frames=int(len(all_P[0])/skip))

    elif numP == 2:
        anim = FuncAnimation(fig, animate_NPP, interval=200, frames=int(len(all_P[0])/skip))

    return anim

def concentration_sum_fig(numP,del_t,psi_2D,all_P,N_2D):
    ...
    ax[0].set_title('Total Phytoplankton Concentration',fontsize=18)
    ax[0].set_xlabel('Time Step (%s s)'%(del_t),fontsize=18)
    ax[0].set_ylabel('$\sum_{i,j} P^n (\mu M N)$',fontsize=18)
    ax[0].legend(fontsize=14)

    ax[1].plot(nut_sum)
    ax[1].set_title('Total Nitrate Concentration',fontsize=18)
